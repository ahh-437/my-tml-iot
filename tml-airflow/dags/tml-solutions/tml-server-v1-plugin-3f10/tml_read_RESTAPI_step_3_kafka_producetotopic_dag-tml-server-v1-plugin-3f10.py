...
##################### INDUSTRIAL API ##############################################################
#-------------------------------- SCADA/MODBUS -----------------------------------------------------                        
        @app.route(rule="/api/v1/scada_modbus_read", methods=["POST"])
        def start_vessel_read():
            ...
            with lock:  # ✅ Thread-safe
                ...
                args=(
                    ...
                    req.get("sendtotopic", ""),            
                    job_id,
                    app.config['VIPERTOKEN'],
                    app.config['VIPERHOST'],
                    app.config['VIPERPORT'],
                    default_args,
        ...
        @app.route(rule="/api/v1/vessel_data", methods=["POST"])
        def vessel_data_callback():
        ...
        @app.route(rule="/api/v1/scada_read_stop", methods=["POST"])
        def stop_vessel_read():
            if sg.read_job:
                sg.read_job["stop"] = True
            return jsonify({"message": "Stop signal sent"}), 200
        
        @app.route(rule="/api/v1/scada_status", methods=["GET"])
        def status():
            return jsonify({
...
################################# MQTT #############################################################
        ...
        @app.route(rule="/api/v1/mqtt_subscribe", methods=["POST"])
        def start_mqtt_subscribe():
          
          print("=== RAW DEBUG ===")
          print("Content-Type:", request.content_type)
          print("Raw body:", repr(request.get_data(as_text=True)))  # repr shows hidden 
  
          try:
            req = request.get_json(force=True,silent=False)
          except Exception as e:
            print("❌ JSON ERROR:", str(e))
            return jsonify({"error": f"JSON parse failed: {str(e)}"}), 400
          ...
          mqtt_cfg = {
            ...
            "port": 8883,#int(req.get("mqtt_port", "8883")),
